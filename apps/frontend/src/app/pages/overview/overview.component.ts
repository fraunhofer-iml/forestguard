import { BatchDto, CompanyDto, UserDto } from '@forrest-guard/api-interfaces';
...
export class OverviewComponent implements AfterViewInit {
   ...
   * @param data The data to get the name from
   * @returns The name of the user or company
   */
  getUserOrCompanyName(data: UserDto | CompanyDto | undefined) {
    if (!data) return '';
    if ((data as UserDto).firstName) {
      data = data as UserDto;
      return `${data.firstName} ${data.lastName}`;
    }
    if ((data as CompanyDto).name) return (data as CompanyDto).name;
    return '';
  }
}
